...
+(function(window, $) {
    ...
    var queryIcon = function(keys) {
        ...
        if(!$search.length) {
            $search = $('<div class="section-search card-content"><div class="loader loading"><i class="icon icon-spin icon-spinner"></i> 姝ｅ湪鎷煎懡鍔犺浇涓�...</div></div>');
    ...
    var openPage = function($section, section, topic) {
        ...
        if(lib) {
            ...
            if(isShowCodeBadage) {
                ...
                if(lib.srcCount) {
                    $dropdown.append('<li class="dropdown-header primary-header" data-type="source"><i class="icon icon-file-code"></i>  婧愮爜 ' + lib.srcCount + '</li>');
                ...
                if(lib.bundlesCount) {
                    $dropdown.append('<li class="dropdown-header primary-header" data-type="bundles"><i class="icon icon-cube"></i> 鎵撳寘</li>');
                    if(lib.bundles.standard) {
                        $dropdown.append('<li class="dropdown-header">鏍囧噯鐗�</li>');
                        var files = [];
                    ...
                    if(lib.bundles.lite) {
                        $dropdown.append('<li class="dropdown-header">绠€娲佺増</li>');
                        var files = [];
                        if(lib.src.js && lib.src.js.length) {
                    ...
                    if(lib.bundles.separate) {
                        $dropdown.append('<li class="dropdown-header">鐙珛缁勪欢</li>');
    ...
    var openSection = function(section, topic) {
        ...
        if(section.url === '') {
            $.zui.messager.show('璇ラ摼鎺ユ墍鎸囩ず鐨勬枃妗ｅ皻鏈畬鎴愩€備綘鍙互Fork椤圭洰鏉ュ畬鍠勬枃妗ｃ€�');
            return;
        }
    ...
    var initCopyable = function() {
        if(!$.zui.browser.isIE() || $.zui.browser.ie > 8) {
            ...
            clipboard.on('success', function(e) {
                ...
                $copyCodeBtn.tooltip('show', '宸插鍒� <i class="icon icon-ok"></i>');
                e.clearSelection();
            });

            clipboard.on('error', function(e) {
                $('#copyCodeTip').addClass('tooltip-warning');
                $copyCodeBtn.tooltip('show', '鎸� <strong>Ctrl+C</strong> 瀹屾垚澶嶅埗');
            });
